...
use crate::{ dto::user_dto::{input::{UpdateUserInput, UpdateUserProfileInput}, output::OwnerUserOutput}, error::AppResult, ports::UserRepository};
...
impl<R: UserRepository> UpdateUserUseCase<R> {
    pub async fn execute(&self, input: UpdateUserInput, state: User) -> AppResult<OwnerUserOutput> {
        ...
        let user_profile = user_profile_builder.build(None,profile.updated_at)?;
