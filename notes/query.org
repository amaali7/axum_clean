...
* 1️⃣ SCHEMA LAYER (typed, reusable, no AST dependency)
** schema/engine.rs
#+begin_src rust
...
* 2️⃣ AST LAYER (pure query meaning)
** ast/expr.rs
#+begin_src rust
...
* 3️⃣ BUILDER LAYER (only builds AST)
...
* 4️⃣ COMPILER LAYER (engine-specific)
** compiler/ctx.rs
#+begin_src rust
...
* 🧠 Why this matches the sub-chat perfectly
| Principle                  | Status |
|----------------------------+--------|
| AST separate from compiler | ✅     |
| Builder produces AST only  | ✅     |
| Engine-agnostic AST        | ✅     |
| Compiler pluggable         | ✅     |
| Transaction as AST         | ✅     |
| Typed schema paths         | ✅     |
...
* ✅ WHERE WE GO NEXT (only when you say “next”)
